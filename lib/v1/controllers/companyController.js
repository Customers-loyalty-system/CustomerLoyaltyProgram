...
const { addCompany, loginService, getCompany, updateCompany, deleteCompany, getAllCompanies, getCompanyMembersService, getCompanyMembersRelationsService } = require("../services/companyServices");
...
const getCompanyMembers = async (req, res, next) => { 
  try {
    const companyMembers = await getCompanyMembersService(req.tokenHolder.id)
    if (companyMembers) return successWithMessage('Your all members', res, companyMembers)
  } catch (err) {
    return failedWithMessage(err.message, res);
  }
}
...
const getCompanyMembersRelations = async (req, res, next) => { 
  try{ 
    const CompanyMembersRelations = await getCompanyMembersRelationsService(req.tokenHolder.id)
    console.log(CompanyMembersRelations)

    return successWithMessage('Your all members', res,CompanyMembersRelations )
  } 
  catch(err) { 
    return failedWithMessage(err.message, res);
  }
