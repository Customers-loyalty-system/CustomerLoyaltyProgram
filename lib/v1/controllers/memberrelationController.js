...
const show = async (req, res, next) => {
  try {
    const relation = await getRelation(req.params.id,req.tokenHolder.id,req.tokenHolder.type);
    if ( relation) return successWithMessage("A member relation have been found",res,relation);
  } catch (err) {
    return failedWithMessage(err.message, res);
  }
};
...
const destroy = async (req, res, next) => {
  try {
    ...
    if (relation) return successWithMessage("A member relation have been deleted",res,relation);
  } catch (err) {
    return failedWithMessage(err.message, res);
  }
};
