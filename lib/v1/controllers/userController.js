...
const index = async (req, res, next) => {
  try {
    ...
    if (users) return successWithMessage("Users found", res, users);
  } catch (err) {
    return failedWithMessage(err.message, res);
  }
};
...
const getUserMemberships = async (req, res, next) => {
  try {
    ...
    return successWithMessage('Your all membershiops', res, userMemberships)
  } catch (err) {
    return failedWithMessage(err.message, res);
  }
};
const getUserMemberRelatios = async (req, res, next) => { 
  try{ 
    const userMemberRelations = await getUserMemberRelatiosService(req.tokenHolder.id)
    return successWithMessage('Your all membershiops', res, userMemberRelations)
  } catch (err) {
    return failedWithMessage(err.message, res);
  }
} 
