...
const genderValidation = [
  body("gender")
    .notEmpty()
    .withMessage("gender can not be empty!").bail()
    .custom((value) => {
...
const titleValidation = [
  body("title")
    .notEmpty()
    .withMessage("title can not be empty!").bail()
    .custom((value) => {
...
const conditiontValdation = [
  body("condition")
    .trim()
    .notEmpty()
    .withMessage("Condition should not be null").bail()
    .isInt()
...
const pointsValdation = [
  body("points")
    .trim()
    .notEmpty()
    .withMessage("Points should not be null").bail()
    .isInt()
...
const configurationsKeysValidation = [
  body("key")
    .notEmpty()
    .withMessage("Configuration can not be empty!").bail()
    .custom((value) => {
      if (!configurationsKeys.includes(value)) {
        throw new Error("Please enter a valid configuration type like: Minimum exchange points, Bronze, Silver, Gold, Platinum");
      }
      return true;
    }),
  errorResponse,
