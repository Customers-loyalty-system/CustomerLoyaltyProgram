...
const getMemberships = async () => {
  try {
    ...
    if (memberships) return membershipsTransformer(memberships);
...
const updateMembershipPoints = async (member, standardPoints, tiresPoints) => {
  try {
    const membership = await await member.update({
      standardPoints,
      tiresPoints,
    });
    if (membership) return membership;
